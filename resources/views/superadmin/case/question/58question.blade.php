    ...
    <div id="Question-58" class="collapse" role="tabpanel" aria-labelledby="heading-58" data-parent="#accordion-2">
        <div class="card-body">
            ...
            <div class="form-group mb-2">
                <input type="radio" id="radioYes58" class="fiftyeightstatus" name="is_trafficking_investigations_q58"
                    value="1" {{ (is_null($q58_checked) || $q58_checked === '1') ? 'checked' : '' }}>
                <label for="radioYes58" class="mr-3">Yes</label>

                <input type="radio" id="radioNo58" class="fiftyeightstatus" name="is_trafficking_investigations_q58"
                    value="0" {{ ($q58_checked === '0') ? 'checked' : '' }}>
                <label for="radioNo58" class="mr-3">No</label>

                <input type="radio" id="radioOthers58" class="fiftyeightstatus" name="is_trafficking_investigations_q58"
                    value="2" {{ ($q58_checked === '2') ? 'checked' : '' }}>
                <label for="radioOthers58">Others</label>
            </div>


            <div id="yes_extra_q58"
                style="display: {{ (is_null($q58_checked) || $q58_checked === '1') ? 'block' : 'none' }};">
                <input type="text" name="trafficking_investigations_title_q58" class="form-control mt-2 q58-yes-input"
                    placeholder="Please describe" value="{{ $q58_data['trafficking_investigations_title_q58'] ?? '' }}">
            </div>


            <div id="others_q58" style="display: {{ ($q58_checked === '2') ? 'block' : 'none' }};">
                <input type="text" name="other_trafficking_investigations_q58"
                    class="form-control mt-2 q58-others-input" placeholder="Others details"
                    value="{{ $q58_data['others'] ?? '' }}">
            </div>
