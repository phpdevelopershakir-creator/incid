...
if (($questiontitles[46]->status ?? null) == 1) {
    ...
    // ডিফল্ট স্ট্যাটিক রো লিস্ট (ডাটাবেজ বা ওল্ড ডাটা না থাকলে এগুলো শো করবে)
    $static_titles = [
        1 => "OEMA 2013",
    ...
    <div id="Question-47" class="collapse" role="tabpanel" aria-labelledby="heading-47" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioFiftySeven1" class="fiftysevenstatus" name="is_government_change_regulated_q47" value="1" {{ $q47_checked == "1" ? "checked" : "" }}>
          <label for="radioFiftySeven1">Yes</label>
        </div>
        
        <div class="icheck-primary">
          <input type="radio" id="radioFiftySeven2" class="fiftysevenstatus" name="is_government_change_regulated_q47" value="0" {{ $q47_checked == "0" ? "checked" : "" }}>
          <label for="radioFiftySeven2">No</label>
        </div>
        
        <div class="icheck-primary input-group">
          <input type="radio" id="radioFiftySeven3" class="fiftysevenstatus" name="is_government_change_regulated_q47" value="2" {{ $q47_checked == "2" ? "checked" : "" }}>
          <label for="radioFiftySeven3">Others</label>
          
          <span class="col-md-6 mt--4 q47_others_container {{ $q47_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q52others" placeholder="Others Specific" class="form-control" value="{{ $q47_others_val }}" name="other_government_change_regulated_q47">
          </span>
        </div>
        ...
        <div id="47_question_view" class="{{ ($q47_checked == '0' || $q47_checked == '2') ? 'visibility' : '' }}">
          <table id="addRowQ47" class="table table-bordered text-center">
            ...
            <tbody>
              <?php 
              // ১. সেশনে পূর্বে সেভ করা ডাটা থাকলে তা লুপে রেন্ডার হবে
              if (!empty($q47_saved_rows)) {
                  ...
                  <tr class="qe47NoOfRow" id="row_q47_<?= $rowIndex ?>">
                    <td>
                      <?php if($is_static) { ?>
                        ...
                        <input type="hidden" name="government_change_regulated_title_q47[]" value="<?= $row['title_val'] ?>" class="q47_title_input">
                      <?php } else { ?>
                        <input type="text" name="government_change_regulated_title_q47[]" value="<?= $row['title_val'] ?>" class="form-control q47_title_input" placeholder="Others Specific">
                      <?php } ?>
                    </td>
                    <td>
                      <select name="government_change_regulated_status_q47[]" class="form-control q47_status_select">
                        ...
                        @foreach ($status_Lists as $key => $training)
                          <option value="{{ $key }}" <?= ($row['status_val'] == $key) ? 'selected' : '' ?>>{{ $training }}</option>
                        @endforeach
                      </select>
                    </td>
                    <td>
                      ...
                      <?php } else { ?>
                        <button type="button" class="btn btn-danger btn_remove_q47" data-id="<?= $rowIndex ?>">-</button>
                      <?php } ?>
                    </td>
                  </tr>
                <?php 
                ...
                // ২. কোনো ব্যাকআপ ডাটা না থাকলে ফ্রেশ ডিফল্ট স্ট্রাকচার লোড হবে
                foreach($static_titles as $key => $title) { ?>
                  <tr class="qe47NoOfRow">
                    <td>
                      <p class="mb-0"><b><?= $title ?></b></p>
                      <input type="hidden" name="government_change_regulated_title_q47[]" value="<?= $key ?>" class="q47_title_input">
                    </td>
                    <td>
                      <select name="government_change_regulated_status_q47[]" class="form-control q47_status_select">
                        <option value="" disabled selected>---Choose an item--</option>
                ...
                <tr class="qe47NoOfRow" id="row_q47_first_dynamic">
                  <td>
                    <input type="text" name="government_change_regulated_title_q47[]" class="form-control q47_title_input" placeholder="Others Specific">
                  </td>
                  <td>
                    <select name="government_change_regulated_status_q47[]" class="form-control q47_status_select">
                      <option value="" disabled selected>---Choose an item--</option>
      ...
      $(document).on("click", '#temp-save-question47', function() {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            alert("Question 47 Draft Saved Temporarily ✅");
          },
          error: function(err) {
