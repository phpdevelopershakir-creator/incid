...
?>
    ...
    <div id="Question-54" class="collapse" role="tabpanel" aria-labelledby="heading-54" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioFiftyFour1" class="fittyfourstatus" name="is_country_diplomats_allegedly_q54" value="1" {{ $q54_checked == "1" ? "checked" : "" }}>
          <label for="radioFiftyFour1">Yes</label>
        </div>
        
        <div class="icheck-primary">
          <input type="radio" id="radioFiftyFour2" class="fittyfourstatus" name="is_country_diplomats_allegedly_q54" value="0" {{ $q54_checked == "0" ? "checked" : "" }}>
          <label for="radioFiftyFour2">No</label>
        </div>
        
        <div class="icheck-primary input-group">
          <input type="radio" id="radioFiftyFour3" class="fittyfourstatus" name="is_country_diplomats_allegedly_q54" value="2" {{ $q54_checked == "2" ? "checked" : "" }}>
          <label for="radioFiftyFour3">Others</label>
          
          <span class="col-md-6 mt--4 q54_others_container {{ $q54_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q54others" placeholder="Others Specific" class="form-control" value="{{ $q54_others_val }}" name="other_country_diplomats_allegedly_q54">
          </span>
        </div>
        ...
        <div id="54_question_view" class="{{ ($q54_checked == '0' || $q54_checked == '2') ? 'visibility' : '' }}">
          <table id="addRowQ54" class="table table-bordered text-center">
            ...
            <tbody>
              <?php 
              // ১. সেশনে পূর্বে সেভ করা ডাটা থাকলে তা লুপ দিয়ে রেন্ডার হবে
              if (!empty($q54_saved_rows)) {
                foreach($q54_saved_rows as $index => $row) { 
                  $country_val = $row['country'] ?? '';
                  // চেক করা হচ্ছে এটা কি ফিক্সড ড্রপডাউন সিলেক্ট ভ্যালু নাকি কাস্টম টেক্সট ইনপুট (Others)
                  ...
                  <tr class="qe54NoOfRow" id="row_q54_<?= $index ?>">
                    <td>
                      <?php if($is_custom_input || $index >= 3) { ?>
                        <input type="text" name="country_diplomat_name_q54[]" value="<?= $country_val ?>" class="form-control q54_country_input" placeholder="Others Specific">
                      <?php } else { ?>
                        <select name="country_diplomat_name_q54[]" class="form-control q54_country_input">
                          ...
                          <?php foreach ($country_Lists as $key => $country_name) { ?>
                            <option value="<?= $key ?>" <?= $country_val == $key ? 'selected' : '' ?>><?= $country_name ?></option>
                          <?php } ?>
                        </select>
                      <?php } ?>
                    </td>
                    <td>
                      <input type="text" name="country_diplomat_description_q54[]" class="form-control country_diplomat_description_q54" value="<?= $row['description'] ?? '' ?>">
                    </td>
                    <td>
                      <input type="number" name="country_diplomat_men_q54[]" class="form-control country_diplomat_men_q54" value="<?= $row['men'] ?? 0 ?>" min="0">
                    </td>
                    <td>
                      <input type="number" name="country_diplomat_women_q54[]" class="form-control country_diplomat_women_q54" value="<?= $row['women'] ?? 0 ?>" min="0">
                    </td>
                    <td>
                      <input type="number" name="country_diplomat_tg_q54[]" class="form-control country_diplomat_tg_q54" value="<?= $row['tg'] ?? 0 ?>" min="0">
                    </td>
                    <td>
                      <input type="number" name="country_diplomat_total_q54[]" class="form-control country_diplomat_total_q54" value="<?= $row['total'] ?? 0 ?>" readonly>
                    </td>
                    <td>
                      <?php if ($index < 3) { ?>
                      ...
                      <?php } else { ?>
                        <button type="button" class="btn btn-danger btn_remove_q54" data-id="<?= $index ?>">-</button>
                      <?php } ?>
                    </td>
                  </tr>
                <?php } 
              } else { 
                // ২. ফ্রেশ অবস্থায় ৩টি ডিফল্ট স্ট্যাটিক রো ড্রপডাউন সহ (কোনো অ্যাকশন বাটন ছাড়া)
                for($k = 0; $k < 3; $k++) { ?>
                ...
                <tr class="qe54NoOfRow" id="row_q54_3">
                  <td>
                    <input type="text" name="country_diplomat_name_q54[]" class="form-control q54_country_input" placeholder="Others Specific">
                  </td>
                  <td><input type="text" name="country_diplomat_description_q54[]" class="form-control country_diplomat_description_q54"></td>
                  <td><input type="number" name="country_diplomat_men_q54[]" class="form-control country_diplomat_men_q54" value="0" min="0"></td>
                  <td><input type="number" name="country_diplomat_women_q54[]" class="form-control country_diplomat_women_q54" value="0" min="0"></td>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      // ➕ নতুন রো যুক্ত করার ডায়নামিক স্ক্রিপ্ট (মাইনাস বাটন সহ যোগ হবে)
      ...
      // 🧮 রিয়েল-টাইমে প্রতি রো-এর সাবটোটাল যোগ করার লজিক
      ...
      // 💾 AJAX ড্রাফট সেভ সাবমিশন (FormData মেথড)
      $(document).on("click", '#temp-save-question54', function() {
        ...
        // লুপ চালিয়ে টেবিলের ডাটা অ্যারে ফরম্যাটে যুক্ত করা
        $('.qe54NoOfRow').each(function(index) {
          ...
          if(country !== '') {
            ...
            formData.append(`question54[q54_table_data][${index}][description]`, description);
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            alert("Question 54 Draft Saved Temporarily ✅");
          },
          error: function(err) {
