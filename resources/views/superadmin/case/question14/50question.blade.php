...
if (($questiontitles[49]->status ?? null) == 1) {
    ...
    // প্রথম ৫টি ফিক্সড টাইটেল ট্র্যাকিং অ্যারে
    $fixed_titles = [
...
?>
    ...
    <div id="Question-50" class="collapse" role="tabpanel" aria-labelledby="heading-50" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioFifty1" class="fiftystatus" name="is_exploitative_treatment_q50" value="1" {{ $q50_checked == "1" ? "checked" : "" }}>
          <label for="radioFifty1">Yes</label>
        </div>
        
        <div class="icheck-primary">
          <input type="radio" id="radioFifty2" class="fiftystatus" name="is_exploitative_treatment_q50" value="0" {{ $q50_checked == "0" ? "checked" : "" }}>
          <label for="radioFifty2">No</label>
        </div>
        
        <div class="icheck-primary input-group">
          <input type="radio" id="radioFifty3" class="fiftystatus" name="is_exploitative_treatment_q50" value="2" {{ $q50_checked == "2" ? "checked" : "" }}>
          <label for="radioFifty3">Others</label>
          
          <span class="col-md-6 mt--4 q50_others_container {{ $q50_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q50others" placeholder="Others Specific" class="form-control" value="{{ $q50_others_val }}" name="others_victim_centered_approach_q50">
          </span>
        </div>
            ...
            <tbody>
              ...
              if (!empty($q50_saved_rows)) {
                  ...
                  <tr class="qe50NoOfRow" id="row_q50_<?= $index ?>">
                    <td>
                      <?php if($is_custom_input || $index >= 5) { ?>
                        <input type="text" name="exploitative_treatment_title_q50[]" value="<?= $title_val ?>" class="form-control q50_title_input" placeholder="Others Specific">
                      <?php } else { ?>
                        <p class="text-left mb-0"><?= $fixed_titles[$title_val] ?? '' ?></p>
                        <input type="hidden" name="exploitative_treatment_title_q50[]" value="<?= $title_val ?>" class="form-control q50_title_input">
                      <?php } ?>
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      <?php } else { ?>
                        <button type="button" class="btn btn-danger btn_remove_q50" data-id="<?= $index ?>">-</button>
                      <?php } ?>
                    </td>
                  </tr>
                <?php } 
              } else { 
                // ২. ফ্রেশ অবস্থায় আপনার কোডের হুবহু ৫টি ডিফল্ট রো (কোনো বাটন ছাড়া)
                foreach($fixed_titles as $key => $title_text) { ?>
                  <tr class="qe50NoOfRow" id="row_q50_<?= $key-1 ?>">
                    <td>
                      <p class="text-left mb-0"><?= $title_text ?></p>
                      <input type="hidden" name="exploitative_treatment_title_q50[]" value="<?= $key ?>" class="form-control q50_title_input">
                    </td>
                ...
                <tr class="qe50NoOfRow" id="row_q50_5">
                  <td>
                    <input type="text" name="exploitative_treatment_title_q50[]" class="form-control q50_title_input" placeholder="Others Specific">
                  </td>
                  <td><input type="file" name="document_upload_q50[]" class="form-control q50_file_input"></td>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      // ➕ নতুন রো যুক্ত করার ডাইনামিক স্ক্রিপ্ট (৬ নম্বর রো এর পর থেকে মাইনাস বাটন সহ যোগ হবে)
      ...
      // 💾 AJAX ড্রাফট সেভ সাবমিশন (FormData ফাইল সাপোর্ট মেথড)
      $(document).on("click", '#temp-save-question50', function() {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            alert("Question 50 Draft Saved Temporarily ✅");
          },
          error: function(err) {
