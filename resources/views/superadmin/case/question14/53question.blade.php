...
if (($questiontitles[52]->status ?? null) == 1) {
    ...
    // প্রথম ৩টি ফিক্সড টাইটেল ট্র্যাকিং অ্যারে
    $fixed_titles = [
...
?>
    ...
    <div id="Question-53" class="collapse" role="tabpanel" aria-labelledby="heading-53" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioFiftyThree1" class="fiftythreestatus" name="is_government_train_diplomat_q53" value="1" {{ $q53_checked == "1" ? "checked" : "" }}>
          <label for="radioFiftyThree1">Yes</label>
        </div>
        
        <div class="icheck-primary">
          <input type="radio" id="radioFiftyThree2" class="fiftythreestatus" name="is_government_train_diplomat_q53" value="0" {{ $q53_checked == "0" ? "checked" : "" }}>
          <label for="radioFiftyThree2">No</label>
        </div>
        
        <div class="icheck-primary input-group">
          <input type="radio" id="radioFiftyThree3" class="fiftythreestatus" name="is_government_train_diplomat_q53" value="2" {{ $q53_checked == "2" ? "checked" : "" }}>
          <label for="radioFiftyThree3">Others</label>
          
          <span class="col-md-6 mt--4 q53_others_container {{ $q53_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q53others" placeholder="Others Specific" class="form-control" value="{{ $q53_others_val }}" name="other_government_train_diplomat_q53">
          </span>
        </div>
            ...
            <tbody>
              ...
              if (!empty($q53_saved_rows)) {
                  ...
                  <tr class="qe53NoOfRow" id="row_q53_<?= $index ?>">
                    <td>
                      <?php if($is_custom_input || $index >= 3) { ?>
                        <input type="text" name="government_title_q53[]" value="<?= $title_val ?>" class="form-control q53_title_input" placeholder="Others Specific">
                      <?php } else { ?>
                        <p class="text-left mb-0"><?= $fixed_titles[$title_val] ?? '' ?></p>
                        <input type="hidden" name="government_title_q53[]" value="<?= $title_val ?>" class="form-control q53_title_input">
                      <?php } ?>
                    </td>
                    <td>
                      <input type="number" name="government_men_q53[]" class="form-control government_men_q53" value="<?= $row['men'] ?? 0 ?>" min="0">
                    </td>
                    <td>
                      <input type="number" name="government_women_q53[]" class="form-control government_women_q53" value="<?= $row['women'] ?? 0 ?>" min="0">
                    </td>
                    <td>
                      <input type="number" name="government_tg_q53[]" class="form-control government_tg_q53" value="<?= $row['tg'] ?? 0 ?>" min="0">
                    </td>
                    <td>
                      <input type="number" name="government_total_q53[]" class="form-control government_total_q53" value="<?= $row['total'] ?? 0 ?>" readonly>
                    </td>
                    <td>
                      <?php if ($index < 3) { ?>
                      ...
                      <?php } else { ?>
                        <button type="button" class="btn btn-danger btn_remove_q53" data-id="<?= $index ?>">-</button>
                      <?php } ?>
                    </td>
                  </tr>
                <?php } 
              } else { 
                // ২. ফ্রেশ অবস্থায় ৩টি ডিফল্ট স্ট্যাটিক রো (কোনো অ্যাকশন বাটন ছাড়া)
                foreach($fixed_titles as $key => $title_text) { ?>
                  <tr class="qe53NoOfRow" id="row_q53_<?= $key-1 ?>">
                    <td>
                      <p class="text-left mb-0"><?= $title_text ?></p>
                      <input type="hidden" name="government_title_q53[]" value="<?= $key ?>" class="form-control q53_title_input">
                    </td>
                ...
                <tr class="qe53NoOfRow" id="row_q53_3">
                  <td>
                    <input type="text" name="government_title_q53[]" class="form-control q53_title_input" placeholder="Others Specific">
                  </td>
                  <td><input type="number" name="government_men_q53[]" class="form-control government_men_q53" value="0" min="0"></td>
                  <td><input type="number" name="government_women_q53[]" class="form-control government_women_q53" value="0" min="0"></td>
                  <td><input type="number" name="government_tg_q53[]" class="form-control government_tg_q53" value="0" min="0"></td>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      // 🧮 বাস্তব সময়ে প্রতি রো-এর টোটাল যোগ করার লজিক
      ...
      $(document).on("click", '#temp-save-question53', function() {
        ...
        // লুপ চালিয়ে টেবিলের প্রতিটি রো-এর ডাটা অ্যারেতে পুশ করা
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            alert("Question 53 Draft Saved Temporarily ✅");
          },
          error: function(err) {
