...
@php
// সেশন থেকে ডাটা তুলে নেওয়া হচ্ছে (১ নম্বরের মতো হুবহু একই নিয়মে)
  ...
  <div id="Question-4" class="collapse" role="tabpanel" aria-labelledby="heading-4" data-parent="#accordion-2">
    <div class="card-body">
      ...
      <div class="icheck-primary">
        <input type="radio" id="radioFour1" class="fourstatus" name="is_supreme_court_q4" value="1" {{ $q4_checked == "1" ? "checked" : "" }}>
        <label for="radioFour1">Yes</label>
      </div>
      <div class="icheck-primary">
        <input type="radio" id="radioFour2" class="fourstatus" name="is_supreme_court_q4" value="0" {{ $q4_checked == "0" ? "checked" : "" }}>
        <label for="radioFour2">No</label>
      </div>
      <div class="icheck-primary input-group">
        <input type="radio" id="radioFour3" class="fourstatus" name="is_supreme_court_q4" value="2" {{ $q4_checked == "2" ? "checked" : "" }}>
        <label for="radioFour3">Others</label>
        <span class="col-md-6 mt--4 q4_others_container {{ $q4_checked == '2' ? '' : 'othersText' }}">
          <input type="text" id="q4others" placeholder="Others Specific" class="form-control" value="{{ $q4_others_val }}" name="others_supreme_court_q4">
        </span>
      </div>
            ...
            <tr class="qe4NoOfRow" id="t1_q4_row_{{ $index }}">
              <td>
                ...
                <select name="supreme_court_title_q4[]" class="form-control">
                  ...
                  <option value="{{ $key }}" {{ ($row['title'] ?? '') == $key ? 'selected' : '' }}>{{ $training }}</option>
                  @endforeach
                </select>
                @else
                <input type="text" name="supreme_court_title_q4[]" class="form-control" placeholder="Others Specific" value="{{ $row['title'] ?? '' }}">
                @endif
              </td>
              <td>
        ...
        <table id="addRow2Q4" class="table table-bordered text-center">
          ...
          <tbody>
            ...
            <tr class="qe4NoOfRow2" id="t2_q4_row_{{ $index }}">
              <td><input type="text" name="supreme_court_title_two_q4[]" class="form-control" value="{{ $row['title_two'] ?? '' }}"></td>
              <td>
                <select name="supreme_court_status_two_q4[]" class="form-control">
                  <option value="" disabled selected>---Choose an item--</option>
                  @foreach ($training_responses_two as $key => $training)
                  <option value="{{ $key }}" {{ ($row['status_two'] ?? '') == $key ? 'selected' : '' }}>{{ $training }}</option>
                  @endforeach
                </select>
              </td>
  ...
  $(document).ready(function() {
    ...
    $("#addRowDatasQ4").click(function() {
      ...
      let htmlRow = `<tr class="qe4NoOfRow" id="t1_q4_row_${t1_counter}">
                <td><input type="text" name="supreme_court_title_q4[]" class="form-control" placeholder="Others Specific"></td>
                <td>
                    <select name="supreme_court_status_q4[]" class="form-control">
                        <option value="" disabled selected>---Choose an item--</option>
    ...
    $(document).on("click", '#temp-save-question4', function() {
      ...
      $('.qe4NoOfRow').each(function() {
        let title = $(this).find('input[name="supreme_court_title_q4[]"]').val() || $(this).find('select[name="supreme_court_title_q4[]"]').val();
