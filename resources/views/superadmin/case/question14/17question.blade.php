...
if (($questiontitles[16]->status ?? null) == 1) {
    // পিওর অ্যারে মেথডে সেশন ডাটা ক্যাচ করা
    ...
    // স্ট্যাটিক ৪টি গাইডের টেক্সট ডিক্লেয়ারেশন
    $static_titles = [
...
?>
    ...
    <div id="Question-17" class="collapse" role="tabpanel" aria-labelledby="heading-17" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioSeventeen1" class="seventeenstatus" name="is_report_country_narrative_protection_q17" value="1" {{ $q17_checked == "1" ? "checked" : "" }}>
          <label for="radioSeventeen1">Yes</label>
        </div>
        <div class="icheck-primary">
          <input type="radio" id="radioSeventeen2" class="seventeenstatus" name="is_report_country_narrative_protection_q17" value="0" {{ $q17_checked == "0" ? "checked" : "" }}>
          <label for="radioSeventeen2">No</label>
        </div>
        <div class="icheck-primary input-group">
          <input type="radio" id="radioSeventeen3" class="seventeenstatus" name="is_report_country_narrative_protection_q17" value="2" {{ $q17_checked == "2" ? "checked" : "" }}>
          <label for="radioSeventeen3">Others</label>
          
          <span class="col-md-6 mt--4 q17_others_container {{ $q17_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q17others" placeholder="Others Specific" class="form-control" value="{{ $q17_others_val }}" name="other_report_country_narrative_protection_q17">
          </span>
        </div>
        ...
        <div id="17_question_view" class="{{ $q17_checked == '0' ? 'visibility' : '' }}">
          <table id="addRowQ17" class="table table-bordered text-center">
            ...
            <tbody>
              <?php 
              // যদি ডাটাবেজ/সেশনে আগের ডাটা সেভ করা থাকে
                  ...
                  <tr class="qe17NoOfRow" id="<?= $unique_row_id ?>">
                    <td>
                      <?php if(isset($row['is_static']) && $row['is_static'] == 1) { ?>
                        ...
                        <input type="hidden" name="report_country_narrative_protection_title_q17[]" value="<?= $row['approach_key'] ?>">
                        <input type="hidden" name="is_static_row_q17[]" value="1">
                      <?php } else { ?>
                        <input type="text" name="report_country_narrative_protection_title_q17[]" class="form-control report_country_narrative_protection_title_q17" placeholder="Others Specific" value="<?= $row['title'] ?? '' ?>">
                    ...
                    <td>
                      <select name="report_country_narrative_protection_description_q17[]" class="form-control q10Input">
                        ...
                        <?php foreach ($Status_Lists as $key => $training) { ?>
                          <option value="<?= $key ?>" <?= (($row['status'] ?? '') == $key) ? 'selected' : '' ?>><?= $training ?></option>
                        <?php } ?>
                      </select>
                    </td>
                    <td><input type="number" name="report_country_narrative_protection_men_q17[]" value="<?= $row['men'] ?? 0 ?>" class="form-control report_country_narrative_protection_men_q17" min="0"></td>
                    <td><input type="number" name="report_country_narrative_protection_women_q17[]" value="<?= $row['women'] ?? 0 ?>" class="form-control report_country_narrative_protection_women_q17" min="0"></td>
                    <td><input type="number" name="report_country_narrative_protection_tg_q17[]" value="<?= $row['tg'] ?? 0 ?>" class="form-control report_country_narrative_protection_tg_q17" min="0"></td>
                    <td><input type="number" name="report_country_narrative_protection_total_q17[]" value="<?= $row['total'] ?? 0 ?>" class="form-control report_country_narrative_protection_total_q17" readonly min="0"></td>
                    <td>
                      <?php if(isset($row['is_static']) && $row['is_static'] == 1) { ?>
                        <?php } else if($index == 4) { ?>
                        <button id="addRowDatasq17" type="button" class="btn btn-sm btn-primary">+</button>
                      <?php } else { ?>
                        <button type="button" class="btn btn-danger btn_remove" data-id="<?= $unique_row_id ?>">-</button>
                      <?php } ?>
                    </td>
                  </tr>
                <?php } 
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      // ➕ নতুন রো অ্যাড করার লজিক (SOP/Guideline এর রিয়েল লেআউট বজায় রেখে)
      ...
      // ➖ ডাইনামিক রো রিমুভ করা
      $(document).on('click', '.btn_remove', function() {
        var targetRowId = $(this).data('id');
        $('#' + targetRowId).remove();
      });

      // 🔢 রিয়েল-টাইম অটো টোটাল সামেশন (Men + Women + TG = Total)
      ...
      // 💾 AJAX ড্রাফট সাবমিশন পে-লোড জেনারেটর
      $(document).on("click", '#temp-save-question17', function() {
        ...
        $('.qe17NoOfRow').each(function() {
          ...
          let status = $(this).find('select[name="report_country_narrative_protection_description_q17[]"]').val();
          let men = $(this).find('input[name="report_country_narrative_protection_men_q17[]"]').val() || 0;
          let women = $(this).find('input[name="report_country_narrative_protection_women_q17[]"]').val() || 0;
          let tg = $(this).find('input[name="report_country_narrative_protection_tg_q17[]"]').val() || 0;
          let total = $(this).find('input[name="report_country_narrative_protection_total_q17[]"]').val() || 0;
          
          let approach_val = "";
          let approach_key = "";

          if(is_static == "1") {
            approach_key = $(this).find('input[name="report_country_narrative_protection_title_q17[]"]').val();
          } else {
            approach_val = $(this).find('input[name="report_country_narrative_protection_title_q17[]"]').val();
          }
