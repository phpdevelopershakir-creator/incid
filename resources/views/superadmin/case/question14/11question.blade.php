...
@php
// সেশন থেকে ১১ নম্বর প্রশ্নের ডাটা অ্যারে হিসেবে তুলে নেওয়া হচ্ছে
$question_11_data = session()->get('question11');

$q11_checked = $question_11_data['q11_checked_value'] ?? "1"; // ডিফল্ট 'Yes'
$q11_rows_data = $question_11_data['q11_data'] ?? null; // ডাইনামিক টেবিল রো ডাটা
  ...
  <div id="Question-11" class="collapse" role="tabpanel" aria-labelledby="heading-11" data-parent="#accordion-2">
    <div class="card-body">
      ...
      <div class="icheck-primary">
        <input type="radio" id="radioEleven1" class="elevenstatus" name="is_government_agreements_transparent_q11" value="1"
          {{ $q11_checked == "1" ? "checked" : "" }}>
        <label for="radioEleven1">Yes</label>
      </div>

      <div class="icheck-primary">
        <input type="radio" id="radioEleven2" class="elevenstatus" name="is_government_agreements_transparent_q11" value="0"
          {{ $q11_checked == "0" ? "checked" : "" }}>
        <label for="radioEleven2">No</label>
      </div>

      <div class="icheck-primary input-group mb-3">
        <input type="radio" id="radioEleven3" class="elevenstatus" name="is_government_agreements_transparent_q11" value="2"
          {{ $q11_checked == "2" ? "checked" : "" }}>
        <label for="radioEleven3">Others</label>

        <span class="col-md-6 mt--4 q11_others_container {{ $q11_checked == '2' ? '' : 'othersText' }}" style="margin-top:-8px;">
      ...
      <div id="11_question_view" class="{{ ($q11_checked == '1') ? '' : 'visibility' }}">
        <table id="addRowQ11" class="table table-bordered text-center">
          ...
          <tbody>
            ...
            <tr class="qe11NoOfRow" id="row{{ $index }}">
              ...
              <td>
                <select name="government_agreements_transparent_status_q11[]" class="form-control">
                  ...
                  <option value="{{ $key }}" {{ ($row['target'] ?? '') == $key ? 'selected' : '' }}>{{ $target }}</option>
                  @endforeach
                </select>
              </td>
              <td>
              ...
              <td>
                @if($index == 0)
                {{-- ১ম রো-তে কোনো অ্যাকশন বাটন নেই --}}
                @elseif($index == 1)
                {{-- ২য় রো-তে প্লাস বাটন থাকবে আপনার মূল কোড অনুযায়ী --}}
                <button id="addRowDatasq11" type="button" class="btn btn-sm btn-primary">+</button>
                @else
                {{-- ৩য় বা তার পরের রোগুলোতে মাইনাস বাটন থাকবে --}}
            ...
            @endforeach
            @else
            {{-- প্রথমবার কোনো সেশন না থাকলে ডিফল্টভাবে এই ২টি রো দেখাবে --}}
            ...
            <tr class="qe11NoOfRow" id="row0">
              ...
              <td>
                <input type="number" name="government_agreements_transparent_total_q11[]" value="0" class="form-control" min="0">
              </td>
              <td></td> {{-- আপনার কোড অনুযায়ী ১ম রো-তে বাটন ফাঁকা --}}
            </tr>

            <tr class="qe11NoOfRow" id="row1">
              <td>
                <input type="text" name="government_agreements_transparent_country_q11[]" class="form-control" placeholder="Others Specific">
              </td>
              <td>
                <select name="government_agreements_transparent_status_q11[]" class="form-control">
                  <option value="" disabled selected>---Choose an item--</option>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    // ⬅️ ডাইনামিক টেবিল রো অ্যাড করার লজিক (৩য় রো থেকে টেক্সট ইনপুট এবং মাইনাস বাটন যোগ হবে)
    $("#addRowDatasq11").click(function() {
      let rowCount = $('.qe11NoOfRow').length + 1;
      let targetOptions = `@foreach ($target_Lists as $key => $target)<option value="{{ $key }}">{{ $target }}</option>@endforeach`;

      $("#addRowQ11 tbody").append(
        '<tr class="qe11NoOfRow" id="row' + rowCount + '">' +
        '<td><input type="text" name="government_agreements_transparent_country_q11[]" class="form-control" placeholder="Others Specific"></td>' +
        '<td><select name="government_agreements_transparent_status_q11[]" class="form-control"><option value="" disabled selected>---Choose an item--</option>' + targetOptions + '</select></td>' +
        '<td><input type="number" name="government_agreements_transparent_total_q11[]" class="form-control" value="0" min="0"></td>' +
        '<td><button type="button" id="' + rowCount + '" class="btn btn-danger btn_remove">-</button></td>' +
        '</tr>'
      );
    });

    // ⬅️ রো রিমুভ করার লজিক
    $(document).on('click', '.btn_remove', function() {
      var button_id = $(this).attr('id');
      $('#row' + button_id).remove();
    });

    // ⬅️ AJAX এর মাধ্যমে মাল্টিপল রো ডাটা সঠিকভাবে সেভ করা
    $(document).on("click", "#temp-save-question11", function() {
      let q11_rows_data = [];

      $('.qe11NoOfRow').each(function() {
        // ১ম রো ড্রপডাউন এবং পরের রোগুলো ইনপুট ফিল্ড—তাই .val() সরাসরি দুই ক্ষেত্র থেকেই ডাটা কালেক্ট করবে
        let country = $(this).find('[name="government_agreements_transparent_country_q11[]"]').val();
        let target = $(this).find('select[name="government_agreements_transparent_status_q11[]"]').val();
        let total = $(this).find('input[name="government_agreements_transparent_total_q11[]"]').val();
      ...
      let saveData = {
        q11_checked_value: $("input[name='is_government_agreements_transparent_q11']:checked").val(),
