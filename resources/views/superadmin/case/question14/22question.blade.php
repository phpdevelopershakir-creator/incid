@if (($questiontitles[21]->status ?? null) == 1)
    @php
        // সেশন থেকে ২২ নম্বর প্রশ্নের ডাটা অ্যারে হিসেবে তুলে নেওয়া হচ্ছে
        ...
        <div id="Question-22" class="collapse" role="tabpanel" aria-labelledby="heading-22" data-parent="#accordion-2">
            <div class="card-body">
                ...
                <div class="icheck-primary input-group mb-3">
                    ...
                    <span class="col-md-6 mt--4 q22_others_container {{ $q22_checked == '2' ? '' : 'othersText' }}" style="margin-top:-8px;">
                ...
                <div id="twenty_two_question_view" class="{{ ($q22_checked == '1') ? '' : 'visibility' }}">
                    <table class="table table-bordered text-center">
                        ...
                        <tbody>
                            @for($i=1; $i<=4; $i++)
                                @php
                                    // সেশন অ্যারে থেকে নির্দিষ্ট ইনপুট ভ্যালু অ্যাসাইন করা (StdClass এরর হ্যান্ডেল করার জন্য)
                                    $men_val = $q22_table_data['q22Men'.$i] ?? 0;
                                    $women_val = $q22_table_data['q22Women'.$i] ?? 0;
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" name="operator_q22[]" id="q22Operator{{$i}}" class="form-control q22Input"
                                            value="{{ $q22_table_data['q22Operator'.$i] ?? '' }}">
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <input type="text" name="capacity_total_q22[]" id="rowTotal{{$i}}" class="form-control q22Input"
                                    ...
                                    <td>
                                        <input type="text" name="eligible_victims_q22[]" id="q22Victims{{$i}}" class="form-control q22Input"
                                            value="{{ $q22_table_data['q22Victims'.$i] ?? '' }}">
                                    </td>
                                    <td>
...
<script>
    ...
    $(document).ready(function() {
        calculateQ22Totals(); // পেজ লোড হবার সময় জেনারেট হওয়া ডাটার টোটাল দেখাবে
    });
...
<script>
    ...
    $(document).on("click", "#temp-save-question22", function() {
        ...
        $.ajax({
            type: "POST",
            url: "/superadmin/case/temp-save-question", // স্ট্যান্ডার্ড ইউনিফর্ম রাউটে ফিক্স করা হয়েছে
            data: {
                _token: "{{ csrf_token() }}",
                question_no: 22,
                question22: {
