  ...
  <div id="Question-9" class="collapse" role="tabpanel" aria-labelledby="heading-9" data-parent="#accordion-2">
    <div class="card-body">
      ...
      <div class="icheck-primary">
        <input type="radio" id="radioNine1" class="ninestatus" name="is_exclusively_trafficking_q9" value="1" {{ $q9_checked == "1" ? "checked" : "" }}>
        <label for="radioNine1">Yes</label>
      </div>
      
      <div class="icheck-primary">
        <input type="radio" id="radioNine2" class="ninestatus" name="is_exclusively_trafficking_q9" value="0" {{ $q9_checked == "0" ? "checked" : "" }}>
        <label for="radioNine2">No</label>
      </div>

      <div class="icheck-primary input-group mb-3">
        <input type="radio" id="radioNine3" class="ninestatus" name="is_exclusively_trafficking_q9" value="2" {{ $q9_checked == "2" ? "checked" : "" }}>
        <label for="radioNine3">Others</label>
        <span class="col-md-6 mt--4 q9_others_container {{ $q9_checked == '2' ? '' : 'othersText' }}" style="margin-top:-8px;">
          <input type="text" id="q9others" placeholder="Others Specific" class="form-control" value="{{ $q9_others_val }}" name="other_exclusively_dedicated_trafficking_q9">
        </span>
      </div>

      <div id="nine_question_view" class="{{ $q9_checked == '1' ? '' : 'visibility' }}">
        <p class="text-muted"><b>Note:</b> If you select 'Yes', a text box will open to provide your opinion.</p>
        ...
        <table id="addRowQ9" class="table table-bordered text-center">
          ...
          <tbody>
            ...
            @php
              $predefined_courts = ['Police Station', 'CID', 'RAB', 'SB', 'Prosecution offices (SPP)', 'Special Tribunal'];
            @endphp
            ...
            @foreach($predefined_courts as $index => $court_name)
              ...
              <tr class="qe9NoOfRow">
                <td style="text-align: left;" class="court-title-cell">
                  {{ $court_name }}
                  <input type="hidden" name="court_title_q9[]" class="field-title" value="{{ $court_name }}">
                </td>
                <td>
                  <div class="radio-group-vertical">
                    <label>
                      <input type="radio" name="row_radio_{{ $index }}" value="1" class="q9-row-radio" {{ $saved_status == '1' ? 'checked' : '' }}> Yes
                    </label>
                    <label>
                      <input type="radio" name="row_radio_{{ $index }}" value="0" class="q9-row-radio" {{ $saved_status == '0' ? 'checked' : '' }}> No
                    </label>
                  </div>
                  <input type="hidden" name="court_type_q9[]" class="court-type-hidden" value="{{ $saved_status }}">
                </td>
                <td>
                  <input type="text" name="court_description_q9[]" class="form-control field-desc desc-td" style="display: {{ $saved_status == '1' ? 'block' : 'none' }};" value="{{ $saved_desc }}">
                </td>
                <td>&nbsp;</td>
              </tr>
            @endforeach
            ...
            @if(!empty($q9_other_rows) && count($q9_other_rows) > 0)
              @foreach($q9_other_rows as $dyn_idx => $other_data)
                ...
                <tr class="qe9NoOfRowOthers">
                  <td>
                    <input type="text" name="court_title_q9[]" class="form-control field-title" value="{{ $other_data['court_title'] ?? '' }}" placeholder="Others Specific">
                  </td>
                  <td>
                    <div class="radio-group-vertical">
                      <label>
                        <input type="radio" name="dyn_radio_{{ $dyn_idx }}" value="1" class="q9-row-radio" {{ $dyn_status == '1' ? 'checked' : '' }}> Yes
                      </label>
                      <label>
                        <input type="radio" name="dyn_radio_{{ $dyn_idx }}" value="0" class="q9-row-radio" {{ $dyn_status == '0' ? 'checked' : '' }}> No
                      </label>
                    </div>
                    <input type="hidden" name="court_type_q9[]" class="court-type-hidden" value="{{ $dyn_status }}">
                  </td>
                  <td>
                    <input type="text" name="court_description_q9[]" class="form-control field-desc desc-td" style="display: {{ $dyn_status == '1' ? 'block' : 'none' }};" value="{{ $other_data['description'] ?? '' }}">
                  </td>
                  <td>
                    @if($loop->first)
            ...
            @else
              ...
              <tr class="qe9NoOfRowOthers">
                <td>
                  <input type="text" name="court_title_q9[]" class="form-control field-title" placeholder="Others Specific">
                </td>
                <td>
                  <div class="radio-group-vertical">
                    <label>
