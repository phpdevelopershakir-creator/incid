...
if (($questiontitles[30]->status ?? null) == 1) {
    // সেশন থেকে পূর্বের সেভ করা ডাটা ক্যাচ করা
...
?>
    ...
    <div id="Question-31" class="collapse" role="tabpane31" aria-labelledby="heading-31" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioThirtyOne1" class="thirtyonestatus" name="is_citizen_victims_abroad_q31" value="1" {{ $q31_checked == "1" ? "checked" : "" }}>
          <label for="radioThirtyOne1">Yes</label>
        </div>
        
        <div class="icheck-primary">
          <input type="radio" id="radioThirtyOne2" class="thirtyonestatus" name="is_citizen_victims_abroad_q31" value="0" {{ $q31_checked == "0" ? "checked" : "" }}>
          <label for="radioThirtyOne2">No</label>
        </div>
        
        <div class="icheck-primary input-group">
          <input type="radio" id="radioThirtyOne3" class="thirtyonestatus" name="is_citizen_victims_abroad_q31" value="2" {{ $q31_checked == "2" ? "checked" : "" }}>
          <label for="radioThirtyOne3">Others</label>
          
          <span class="col-md-6 mt--4 q31_others_container {{ $q31_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q31others" placeholder="Others Specific" class="form-control" value="{{ $q31_others_val }}" name="other_citizen_victims_abroad_q31">
          </span>
        </div>
        ...
        <div id="31_question_view" class="{{ ($q31_checked == '0' || $q31_checked == '2') ? 'visibility' : '' }}">
          <table id="addRowQ31" class="table table-bordered text-center">
            ...
            <tbody>
              <?php 
              // এখানে লুপটি ফিক্সড ৪ বার (০ থেকে ৩) চলবেই চলবে, প্লাস অতিরিক্ত রো থাকলে সেগুলো আসবে
                  ...
                  <tr class="qe31NoOfRow" id="row_q31_<?= $index ?>">
                    <td>
                      <?php if($is_custom_input) { ?>
                        <input type="text" name="citizen_victims_abroad_name_q31[]" value="<?= $country_val ?>" class="form-control q31_country_input" placeholder="Others Specific">
                      <?php } else { ?>
                        <select name="citizen_victims_abroad_name_q31[]" class="form-control q31_country_input">
                          <option value="" disabled <?= empty($country_val) ? 'selected' : '' ?>>---Choose an item--</option>
                          <?php foreach ($country_Lists as $key => $country_name) { ?>
                            <option value="<?= $country_name ?>" <?= $country_val == $country_name ? 'selected' : '' ?>><?= $country_name ?></option>
                          <?php } ?>
                        </select>
                      <?php } ?>
                    </td>
                   ...
                   <td>
                    <select name="citizen_victims_abroad_status_q31[]" class="form-control q31_status_input">
                        <option value="" disabled <?= empty($status_val) ? 'selected' : '' ?>>---Choose an item--</option>
                        <?php foreach ($status_Lists as $key => $status_name) { ?>
                          <option value="<?= $status_name ?>" <?= $status_val == $status_name ? 'selected' : '' ?>><?= $status_name ?></option>
                        <?php } ?>
                    </select>
                  </td>
                    <td><input type="number" name="citizen_victims_abroad_men_q31[]" class="form-control citizen_victims_abroad_men_q31" value="<?= $row['men'] ?? 0 ?>" min="0"></td>
                    <td><input type="number" name="citizen_victims_abroad_women_q31[]" class="form-control citizen_victims_abroad_women_q31" value="<?= $row['women'] ?? 0 ?>" min="0"></td>
                    <td><input type="number" name="citizen_victims_abroad_tg_q31[]" class="form-control citizen_victims_abroad_tg_q31" value="<?= $row['tg'] ?? 0 ?>" min="0"></td>
                    <td><input type="number" name="citizen_victims_abroad_boy_q31[]" class="form-control citizen_victims_abroad_boy_q31" value="<?= $row['boy'] ?? 0 ?>" min="0"></td>
                    <td><input type="number" name="citizen_victims_abroad_girl_q31[]" class="form-control citizen_victims_abroad_girl_q31" value="<?= $row['girl'] ?? 0 ?>" min="0"></td>
                    <td><input type="number" name="citizen_victims_abroad_total_q31[]" class="form-control citizen_victims_abroad_total_q31" value="<?= $row['total'] ?? 0 ?>" readonly></td>
                    <td>
                      <?php 
                      // বাটন লজিক একদম ফিক্সড এবং ক্লিয়ার
                      if ($index < 3) {
                          // প্রথম ৩টি রো-তে কোনো অ্যাকশন বাটন থাকবে না
                          echo '<span class="text-muted">-</span>';
                      } elseif ($index == 3) {
                          // ৪ নম্বর রো-তে সবসময় প্লাস (+) বাটন থাকবেই
                          echo '<button id="addRowDatasq31" type="button" class="btn btn-sm btn-primary">+</button>';
                      } else {
                          // ৪ নম্বরের পরের যেকোনো অতিরিক্ত রো-তে মাইনাস (-) বাটন থাকবে
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      // ➕ নতুন রো যুক্ত করার ডায়নামিক স্ক্রিপ্ট
      ...
      $(document).on("click", "#addRowDatasq31", function() {
        ...
        // নতুন রো সবসময় টেবিলের একদম শেষে যুক্ত হবে
        $("#addRowQ31 tbody").append(
          `<tr class="qe31NoOfRow" id="row_q31_${dynamicRowCount}">
            <td>
              <input type="text" name="citizen_victims_abroad_name_q31[]" class="form-control q31_country_input" placeholder="Others Specific">
            </td>
            <td>
              <select name="citizen_victims_abroad_status_q31[]" class="form-control q31_status_input">
                ${statusOptions}
      ...
      // 🧮 রিয়েল-টাইমে যোগফল বের করা
      $(document).on('input', '.citizen_victims_abroad_men_q31, .citizen_victims_abroad_women_q31, .citizen_victims_abroad_tg_q31, .citizen_victims_abroad_boy_q31, .citizen_victims_abroad_girl_q31', function() {
