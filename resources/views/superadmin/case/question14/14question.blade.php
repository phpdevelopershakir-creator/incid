...
if (($questiontitles[13]->status ?? null) == 1) {
    // ১ নম্বরের মতো পিওর অ্যারে মেথডে সেশন ডাটা ক্যাচ করা
    ...
    // স্ট্যাটিক ৪টি রো-এর গাইডলাইন নাম
    $static_guidelines = [
...
?>
    ...
    <div id="Question-14" class="collapse" role="tabpanel" aria-labelledby="heading-14" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioFourteen1" class="fourteen_status" name="is_government_devote_implement_q14" value="1" {{ $q14_checked == "1" ? "checked" : "" }}>
          <label for="radioFourteen1">Yes</label>
        </div>
        <div class="icheck-primary">
          <input type="radio" id="radioFourteen2" class="fourteen_status" name="is_government_devote_implement_q14" value="0" {{ $q14_checked == "0" ? "checked" : "" }}>
          <label for="radioFourteen2">No</label>
        </div>
        <div class="icheck-primary input-group">
          <input type="radio" id="radioFourteen3" class="fourteen_status" name="is_government_devote_implement_q14" value="2" {{ $q14_checked == "2" ? "checked" : "" }}>
          <label for="radioFourteen3">Others</label> 
          
          <span class="col-md-6 mt--4 q14_others_container {{ $q14_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q14others" placeholder="Others Specific" class="form-control" value="{{ $q14_others_val }}" name="other_government_devote_implement_q14">
          </span>
        </div>
        ...
        <div id="14_question_view" class="{{ $q14_checked == '0' ? 'visibility' : '' }}">
          <table id="addRowQ14" class="table table-bordered text-center">
            ...
            <tbody>
              <?php 
              // যদি সেশনে আগের সেভ করা ডাটা থাকে
                ...
                foreach($q14_table_data as $index => $row) { 
                  ...
                  <tr class="qe14NoOfRow" id="<?= $unique_row_id ?>">
                    <td>
                      ...
                      <?php } else { ?>
                        <input type="text" name="original_approach_q14[]" class="form-control original_approach_q14" placeholder="Others Specific" value="<?= $row['title'] ?? '' ?>">
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      // Yes/No/Others রেডিও কন্ডিশনাল টগল লজিক
      ...
      // ➕ নতুন রো অ্যাড করা (Unique ID জেনারেট করে মাইনাস বাটন হ্যান্ডেল করা হয়েছে)
      ...
      // ➖ ডাইনামিক অ্যাড হওয়া রো রিমুভ করা
      $(document).on('click', '.btn_remove', function() {
        var targetRowId = $(this).data('id');
        $('#' + targetRowId).remove();
      });

      // 💾 AJAX ড্রাফট সেশন সেভ সাবমিশন লজিক (১ নম্বরের আদলে তৈরি)
      $(document).on("click", '#temp-save-question14', function() {
        ...
        $('.qe14NoOfRow').each(function() {
          ...
          if(is_static == "1") {
            // স্ট্যাটিক ৪টির জন্য কী সংরক্ষণ করা হচ্ছে
            approach_key = $(this).find('input[name="original_approach_q14[]"]').val();
          } else {
            // ডাইনামিক টেক্সট বক্স এর ইনপুট ভ্যালু ক্যাচ করা
