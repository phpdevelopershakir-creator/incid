...
@php
// সেশন থেকে ২১ নম্বর প্রশ্নের ডাটা অ্যারে হিসেবে তুলে নেওয়া হচ্ছে
  ...
  <div id="Question-21" class="collapse" role="tabpanel" aria-labelledby="heading-21" data-parent="#accordion-2">
    <div class="card-body">
      ...
      <div class="icheck-primary input-group mb-3">
        ...
        <span class="col-md-6 mt--4 q21_others_container {{ $q21_checked == '2' ? '' : 'othersText' }}" style="margin-top:-8px;">
      ...
      <div id="twentyone_question_view" class="{{ ($q21_checked == '1') ? '' : 'visibility' }}">
        <table class="table table-bordered text-center">
          ...
          <tbody>
            @for($i=1; $i<=4; $i++)
              @php
              // সেশন অ্যারে থেকে নির্দিষ্ট ইনপুট ভ্যালু অ্যাসাইন করা
              $men_val=$q21_table_data['q21Men'.$i] ?? 0;
              $women_val=$q21_table_data['q21Women'.$i] ?? 0;
              $row_total=(int)$men_val + (int)$women_val;
              @endphp
              <tr>
              <td>
              ...
              <td>
                <input type="text" name="operator_q21[]" id="q21Operator{{$i}}" class="form-control q21Input"
                  value="{{ $q21_table_data['q21Operator'.$i] ?? '' }}">
              </td>
              <td>
                <input type="number" name="capacity_men_q21[]" id="q21Men{{$i}}" class="form-control question21rowmen q21Input"
                  value="{{ $men_val }}" min="0">
              </td>
              <td>
                <input type="number" name="capacity_women_q21[]" id="q21Women{{$i}}" class="form-control question21rowWomen q21Input"
                  value="{{ $women_val }}" min="0">
              </td>
              <td>
                <input type="text" name="capacity_total_q21[]" id="rowTotal{{$i}}" class="form-control q21Input"
                  value="{{ $row_total }}" readonly>
              </td>
              <td>
                <input type="text" name="is_specialized_q21[]" id="q21Trafficking{{$i}}" class="form-control q21Input"
                  value="{{ $q21_table_data['q21Trafficking'.$i] ?? '' }}">
              </td>
              <td>
                <input type="text" name="eligible_victims_q21[]" id="q21Victims{{$i}}" class="form-control q21Input"
                  value="{{ $q21_table_data['q21Victims'.$i] ?? '' }}">
              </td>
              <td>
...
<script>
  // ⬅️ রো ভিত্তিক যোগফল এবং গ্র্যান্ড টোটাল ক্যালকুলেশন ফাংশন
  ...
  // ইনপুট দেওয়ার সাথে সাথে লাইভ ক্যালকুলেশন ট্রিপ করা
  ...
  $(document).ready(function() {
    calculateQ21Totals(); // পেজ লোড হবার সময় জেনারেট হওয়া ডাটার টোটাল দেখাবে
  });
...
<script>
  // ⬅️ রেডিও বাটন হাইড/শো হ্যান্ডেলার
  $(document).ready(function() {
...
<script>
  // ⬅️ AJAX এর মাধ্যমে ফর্ম ডাটা অবজেক্ট তৈরি করে টেম্পোরারি সেভ করা
  $(document).on("click", "#temp-save-question21", function() {
    ...
    $.ajax({
      type: "POST",
      url: "/superadmin/case/temp-save-question", // স্ট্যান্ডার্ড রাউটে ফিক্স করা হয়েছে
      data: {
        _token: "{{ csrf_token() }}",
        question_no: 21,
        question21: {
