...
if (($questiontitles[54]->status ?? null) == 1) {
    // সেশন থেকে ৫৫ নম্বর প্রশ্নের সেভ করা ডাটা ক্যাচ করা
...
?>
    ...
    <div id="Question-55" class="collapse" role="tabpanel" aria-labelledby="heading-55" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioFiftyFive1" class="fiftyfivestatus" name="is_government_provide_trafficking_q55" value="1" {{ $q55_checked == "1" ? "checked" : "" }}>
          <label for="radioFiftyFive1">Yes</label>
        </div>
        
        <div class="icheck-primary">
          <input type="radio" id="radioFiftyFive2" class="fiftyfivestatus" name="is_government_provide_trafficking_q55" value="0" {{ $q55_checked == "0" ? "checked" : "" }}>
          <label for="radioFiftyFive2">No</label>
        </div>
        
        <div class="icheck-primary input-group">
          <input type="radio" id="radioFiftyFive3" class="fiftyfivestatus" name="is_government_provide_trafficking_q55" value="2" {{ $q55_checked == "2" ? "checked" : "" }}>
          <label for="radioFiftyFive3">Others</label>
          
          <span class="col-md-6 mt--4 q55_others_container {{ $q55_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q55others" placeholder="Others Specific" class="form-control" value="{{ $q55_others_val }}" name="other_government_provide_trafficking_q55">
          </span>
        </div>
        ...
        <div id="55_question_view" class="{{ ($q55_checked == '0' || $q55_checked == '2') ? 'visibility' : '' }}">
          <table id="addRowQ55" class="table table-bordered text-center">
            ...
            <tbody>
              <?php 
              // ১. সেশনে ডাটা থাকলে তা লুপ দিয়ে রেন্ডার হবে
              if (!empty($q55_saved_rows)) {
                  ...
                  <tr class="qe55NoOfRow" id="row_q55_<?= $index ?>">
                    <td>
                      <?php if($is_custom_input || $index >= 3) { ?>
                        <input type="text" name="government_provide_name_q55[]" value="<?= $country_val ?>" class="form-control q55_country_input" placeholder="Others Specific">
                      <?php } else { ?>
                    ...
                    <td>
                      <input type="text" name="government_provide_description_q55[]" class="form-control government_provide_description_q55" value="<?= $row['description'] ?? '' ?>">
                    </td>
                    <td>
                      <input type="number" name="government_provide_men_q55[]" class="form-control government_provide_men_q55" value="<?= $row['men'] ?? 0 ?>" min="0">
                    </td>
                    <td>
                      <input type="number" name="government_provide_women_q55[]" class="form-control government_provide_women_q55" value="<?= $row['women'] ?? 0 ?>" min="0">
                    </td>
                    <td>
                      <input type="number" name="government_provide_tg_q55[]" class="form-control government_provide_tg_q55" value="<?= $row['tg'] ?? 0 ?>" min="0">
                    </td>
                    <td>
                      <input type="number" name="government_provide_total_q55[]" class="form-control government_provide_total_q55" value="<?= $row['total'] ?? 0 ?>" readonly>
                    </td>
                    <td>
                      <?php if ($index < 3) { ?>
                      ...
                      <?php } else { ?>
                        <button type="button" class="btn btn-danger btn_remove_q55" data-id="<?= $index ?>">-</button>
                      <?php } ?>
                    </td>
                  </tr>
                <?php } 
              } else { 
                // ২. ফ্রেশ অবস্থায় ৩টি ডিফল্ট ফিক্সড রো ড্রপডাউন সহ
                for($k = 0; $k < 3; $k++) { ?>
                ...
                <tr class="qe55NoOfRow" id="row_q55_3">
                  <td>
                    <input type="text" name="government_provide_name_q55[]" class="form-control q55_country_input" placeholder="Others Specific">
                  </td>
                  <td><input type="text" name="government_provide_description_q55[]" class="form-control government_provide_description_q55"></td>
                  <td><input type="number" name="government_provide_men_q55[]" class="form-control government_provide_men_q55" value="0" min="0"></td>
                  <td><input type="number" name="government_provide_women_q55[]" class="form-control government_provide_women_q55" value="0" min="0"></td>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      // ➕ নতুন ডায়নামিক রো যুক্ত করার স্ক্রিপ্ট (টাইপ নাম্বার ফিক্সড করা হয়েছে)
      ...
      // 🧮 রিয়েল-টাইমে সাবটোটাল ক্যালকুলেশন
      ...
      $(document).on("click", '#temp-save-question55', function() {
        ...
        // লুপ চালিয়ে টেবিলের সব ডাটা অ্যারে ফরম্যাটে ফর্ম-ডাটাতে পুশ করা
        $('.qe55NoOfRow').each(function(index) {
          ...
          if(country !== '') {
            ...
            formData.append(`question55[q55_table_data][${index}][description]`, description);
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            alert("Question 55 Draft Saved Temporarily ✅");
          },
          error: function(err) {
