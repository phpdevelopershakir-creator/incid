...
?>
    ...
    <div id="Question-49" class="collapse" role="tabpanel" aria-labelledby="heading-49" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioFoutyNine1" class="fourtyninestatus" name="is_government_agreements_transparent_q49" value="1" {{ $q49_checked == "1" ? "checked" : "" }}>
          <label for="radioFoutyNine1">Yes</label>
        </div>
        
        <div class="icheck-primary">
          <input type="radio" id="radioFourtyNine2" class="fourtyninestatus" name="is_government_agreements_transparent_q49" value="0" {{ $q49_checked == "0" ? "checked" : "" }}>
          <label for="radioFourtyNine2">No</label>
        </div>
        
        <div class="icheck-primary input-group">
          <input type="radio" id="radioFourtyNine3" class="fourtyninestatus" name="is_government_agreements_transparent_q49" value="2" {{ $q49_checked == "2" ? "checked" : "" }}>
          <label for="radioFourtyNine3">Others</label>
          
          <span class="col-md-6 mt--4 q49_others_container {{ $q49_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q49others" placeholder="Others Specific" class="form-control" value="{{ $q49_others_val }}" name="other_government_agreements_transparent_q49">
          </span>
        </div>
        ...
        <div id="49_question_view" class="{{ ($q49_checked == '0' || $q49_checked == '2') ? 'visibility' : '' }}">
          <table id="addRowQ49" class="table table-bordered text-center">
            ...
            <tbody>
              <?php 
              // ১. যদি সেশনে পূর্বে সেভ করা ডাটা থাকে, তবে তা সিরিয়ালি রেন্ডার হবে
              if (!empty($q49_saved_rows)) {
                  ...
                  <tr class="qe49NoOfRow" id="row_q49_<?= $index ?>">
                    <td>
                      <?php if($is_custom_input || $index >= 3) { ?>
                        <input type="text" name="government_agreements_transparent_country_q49[]" value="<?= $country_val ?>" class="form-control q49_country_input" placeholder="Others Specific">
                      <?php } else { ?>
                        <select name="government_agreements_transparent_country_q49[]" class="form-control q49_country_input">
                          <option value="" disabled selected>---Choose an item--</option>
                          @foreach ($country_Lists as $key => $country)
                            <option value="{{ $key }}" <?= ($country_val == $key) ? 'selected' : '' ?>>{{ $country }}</option>
                          @endforeach
                        </select>
                      <?php } ?>
                    </td>
                    <td>
                      <select name="government_agreements_transparent_status_q49[]" class="form-control q49_instrument_select">
                        ...
                        @foreach ($Instruments_Lists as $key => $instrument)
                          <option value="{{ $key }}" <?= (($row['instrument'] ?? '') == $key) ? 'selected' : '' ?>>{{ $instrument }}</option>
                        @endforeach
                      </select>
                    </td>
                    <td>
                      ...
                      <?php if(!empty($row['uploaded_file_name'])) { ?>
                        <small class="text-success d-block text-left mt-1">✓ Current: <?= $row['uploaded_file_name'] ?></small>
                      <?php } ?>
                    </td>
                    <td>
                      <?php if ($index < 3) { ?>
                      ...
                      <?php } else { ?>
                        <button type="button" class="btn btn-danger btn_remove_q49" data-id="<?= $index ?>">-</button>
                      <?php } ?>
                    </td>
                  </tr>
                <?php } 
              } else { 
                // ২. ফ্রেশ অবস্থায় আপনার কোডের হুবহু ৩টি ডিফল্ট সিলেক্ট রো (কোনো বাটন ছাড়া)
                for ($i = 0; $i < 3; $i++) { ?>
                ...
                <tr class="qe49NoOfRow" id="row_q49_3">
                  <td>
                    <input type="text" name="government_agreements_transparent_country_q49[]" class="form-control q49_country_input" placeholder="Others Specific">
                  </td>
                  <td>
                    <select name="government_agreements_transparent_status_q49[]" class="form-control q49_instrument_select">
                      <option value="" disabled selected>---Choose an item--</option>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      // ➕ নতুন রো যুক্ত করার ডায়নামিক স্ক্রিপ্ট (৪ নম্বর রো এর পর থেকে মাইনাস বাটন সহ যোগ হবে)
      ...
      $(document).on("click", '#temp-save-question49', function() {
        ...
        // লুপ চালিয়ে প্রতিটি রো-এর ডাটা অ্যারেতে সাজানো
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            alert("Question 49 Draft Saved Temporarily ✅");
          },
          error: function(err) {
