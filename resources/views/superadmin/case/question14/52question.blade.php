    ...
    <div id="Question-52" class="collapse" role="tabpanel" aria-labelledby="heading-52" data-parent="#accordion-2">
      <div class="card-body">
        ...
        <div class="icheck-primary">
          <input type="radio" id="radioFiftyTwo1" class="fiftytwostatus" name="is_government_prosecute_deport_q52" value="1" {{ $q52_checked == "1" ? "checked" : "" }}>
          <label for="radioFiftyTwo1">Yes</label>
        </div>
        
        <div class="icheck-primary">
          <input type="radio" id="radioFiftyTwo2" class="fiftytwostatus" name="is_government_prosecute_deport_q52" value="0" {{ $q52_checked == "0" ? "checked" : "" }}>
          <label for="radioFiftyTwo2">No</label>
        </div>
        
        <div class="icheck-primary input-group">
          <input type="radio" id="radioFiftyTwo3" class="fiftytwostatus" name="is_government_prosecute_deport_q52" value="2" {{ $q52_checked == "2" ? "checked" : "" }}>
          <label for="radioFiftyTwo3">Others</label>
          
          <span class="col-md-6 mt--4 q52_others_container {{ $q52_checked == '2' ? '' : 'othersText' }}">
            <input type="text" id="q52others" placeholder="Others Specific" class="form-control" value="{{ $q52_others_val }}" name="other_government_prosecute_deport_q52">
          </span>
        </div>
        ...
        <div id="52_question_view" class="{{ ($q52_checked == '0' || $q52_checked == '2') ? 'visibility' : '' }}">
          <table id="addRowQ52" class="table table-bordered text-center">
            ...
            <tbody>
              <?php 
              // ১. সেশনে পূর্বে সেভ করা ডাটা থাকলে তা সিরিয়ালি রেন্ডার হবে
              if (!empty($q52_saved_rows)) {
                  ...
                  <tr class="qe52NoOfRow" id="row_q52_<?= $index ?>">
                    <td>
                      <?php if($is_custom_input || $index >= 3) { ?>
                        <input type="text" name="prosecute_title_q52[]" value="<?= $title_val ?>" class="form-control q52_title_input" placeholder="Others Specific">
                      <?php } else { ?>
                        <p class="text-left mb-0"><?= $fixed_titles[$title_val] ?? '' ?></p>
                        <input type="hidden" name="prosecute_title_q52[]" value="<?= $title_val ?>" class="form-control q52_title_input">
                      <?php } ?>
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      <?php } else { ?>
                        <button type="button" class="btn btn-danger btn_remove_q52" data-id="<?= $index ?>">-</button>
                      <?php } ?>
                    </td>
                  </tr>
                <?php } 
              } else { 
                // ২. ফ্রেশ অবস্থায় ৩টি ডিফল্ট স্ট্যাটিক রো (কোনো বাটন ছাড়া)
                foreach($fixed_titles as $key => $title_text) { ?>
                  <tr class="qe52NoOfRow" id="row_q52_<?= $key-1 ?>">
                    <td>
                      <p class="text-left mb-0"><?= $title_text ?></p>
                      <input type="hidden" name="prosecute_title_q52[]" value="<?= $key ?>" class="form-control q52_title_input">
                    </td>
                    <td>
                      <select name="prosecute_status_q52[]" class="form-control q52_status_select">
                        <option value="">---Choose---</option>
                ...
                <tr class="qe52NoOfRow" id="row_q52_3">
                  <td>
                    <input type="text" name="prosecute_title_q52[]" class="form-control q52_title_input" placeholder="Others Specific">
                  </td>
                  <td>
                    <select name="prosecute_status_q52[]" class="form-control q52_status_select">
                      <option value="">---Choose---</option>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      $(document).on("click", '#temp-save-question52', function() {
        ...
        // লুপ চালিয়ে প্রতিটি রো-এর ডাটা অ্যারেতে পুশ করা
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            alert("Question 52 Draft Saved Temporarily ✅");
          },
          error: function(err) {
