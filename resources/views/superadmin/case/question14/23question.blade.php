...
<div class="card question23">
  ...
  <div id="Question-23" class="collapse" role="tabpane23" aria-labelledby="heading-22" data-parent="#accordion-2">
    <div class="card-body">
      ...
      <input type="radio" id="radioOthers_q23" class="twentythreestatus" name="is_complicit_official_q23" value="2"
        {{ ($q23_checked === '2') ? 'checked' : '' }}>
      <label for="radioOthers_q23">Others</label>

      <div id="others_q23" style="display: {{ ($q23_checked === '2') ? 'block' : 'none' }};">
        <input type="text" name="others_complicit_official_q23" class="form-control mt-2 q23-others-input" placeholder="Others details"
          value="{{ $q23_data['others'] ?? '' }}">
      </div>

      <div id="yes_extra_q23" style="display: {{ (is_null($q23_checked) || $q23_checked === '1') ? 'block' : 'none' }};">
        <input type="text" name="involved_directly_trafficking_title_q23" class="form-control mt-2 q23-yes-input"
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.success) {
          ...
          alert("Question 23 has been saved temporarily");
        } else {
          alert("Not Saved");
        }
      },
